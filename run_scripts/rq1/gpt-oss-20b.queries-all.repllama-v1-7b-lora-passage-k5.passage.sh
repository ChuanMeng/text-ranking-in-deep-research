...
INDEX_PATH=./indexes/index.repllama-v1-7b-lora-passage.passage/corpus.pkl
...
OUT_DIR=./runs/${MODEL_NAME}-${REASONING_EFFORT}/${QUERY_NAME}.repllama-v1-7b-lora-passage-k${K}.${CORPUS_TYPE}-psgid
OUT_DOC_DIR=./runs/${MODEL_NAME}-${REASONING_EFFORT}/${QUERY_NAME}.repllama-v1-7b-lora-passage-k${K}.${CORPUS_TYPE}
...
python search_agent/oss_client.py \
  ...
  --lora-name castorini/repllama-v1-7b-lora-passage \
  --task-prefix "query: " \
  --torch-dtype "bfloat16" \
  --pooling eos --append-eos-token \
  --normalize \
  --query ${QUERY_FILE} \
